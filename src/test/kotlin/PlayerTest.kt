...
internal class PlayerTest {
    ...
    internal fun `should print correct winner message given winning Player`(player: Player, winnerMessage: String) {
        assertEquals(winnerMessage, player.getWinnerMessage())
    }

    @ParameterizedTest
    @CsvSource("HUMAN; You are the predictor, what is your input?", "AI; AI is the predictor, what is your input?", delimiter = ';')
    internal fun `should print correct ask for input message given current predicting Player`(player: Player, askForInputMessage: String) {
        assertEquals(askForInputMessage, player.getAskForInputMessage())
    }
